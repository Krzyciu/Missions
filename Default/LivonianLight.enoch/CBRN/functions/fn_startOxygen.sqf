...
if (isNull (uiNamespace getVariable ["cbrn_o2", objNull])) then {
    ...
    if !(isNull _display) then {
        ...
        _ctrl ctrlSetStructuredText parseText format ["<t color='#%1' align='left' valign='top' size='1.2'>Dopływ O²</t>", _color];
...
[{
    ...
    if !(_oldBackpack isEqualTo _backpack) then {
        systemChat format ["Podłączono nową butlę z %1%2 powietrza!", round ((_curOxygen/cbrn_maxOxygenTime) * 100), "%"];
        ...
        (uiNamespace getVariable ["cbrn_o2", ctrlNull]) ctrlSetStructuredText parseText format ["<t color='#%1' align='left' valign='top' size='1.2'>Dopływ O²</t>", _color];
    ...
    if (!(_backpack getVariable ["cbrn_5min_warning", false]) && {_reserve < 300 && _reserve >= 30 }) then {
        ...
        (uiNamespace getVariable ["cbrn_o2", ctrlNull]) ctrlSetStructuredText parseText "<t color='#ffa500' align='left' valign='top' size='1.2'>Dopływ O²</t>";
        [{
            detach _this;
            deleteVehicle _this;
    ...
    if (!(_backpack getVariable ["cbrn_1min_warning", false]) && {_reserve < 30}) then {
        ...
        (uiNamespace getVariable ["cbrn_o2", ctrlNull]) ctrlSetStructuredText parseText "<t color='#ff0000' align='left' valign='top' size='1.2'>Dopływ O²</t>";
        [{
            detach _this;
            deleteVehicle _this;
