...
while {!isNull _leak_obj} do
{
	...
	_puddle_leak attachTo [_leak_obj,[0,0,-2.9]];
	waitUntil {sleep 2; player distance _puddle_leak < 300};

	_bubble_leak = "#particlesource" createVehicleLocal (getPosASL _leak_obj);
	...
	_bubble_leak setDropInterval 0.01;

	//water leak
	_wet_leak = "#particlesource" createVehicleLocal (getPosASL _leak_obj);
	...
	//stropi jos
	_stropi = "#particlesource" createVehicleLocal (getPosASL _puddle_leak);
	...
	//splash jos
	_splash_wave = "#particlesource" createVehicleLocal (getPosASL _puddle_leak);
	...
	_impact = "#particlesource" createVehicleLocal (getPosASL _puddle_leak);
	...
	_drop_up = "#particlesource" createVehicleLocal (getPosASL _puddle_leak);
	...
	//splash jos
	_splash_r = "#particlesource" createVehicleLocal (getPosASL _puddle_leak);
	...
	_mission_wetFX = "#particlesource" createVehicleLocal (getPosASL _puddle_leak);
	_mission_wetFX setParticleClass "ACERainEffect";
	_mission_wetFX setDropInterval 0.1;

	while {player distance _puddle_leak < 100} do {_puddle_leak say3D ["waterDrop", 30]; sleep 7+(random 2)};
